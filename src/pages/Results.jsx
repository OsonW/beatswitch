...
export default function Results({ user }) {
  ...
  // Params come from router navigation state, stable for this navigation, so deps are intentionally empty.
  useEffect(() => {
    let active = true
    generatePlaylist(moodParams)
      .then(t => { if (active) { setTracks(t); setLoading(false) } })
  ...
  const handleReshuffle = async () => {
    ...
    setTimeout(() => {
      setTracks(prev => {
        ...
        let ui = 0
        prev.forEach(t => {
          if (pins.has(t.name)) result.push(t)
